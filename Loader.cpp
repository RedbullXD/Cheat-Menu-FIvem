...
{
    WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(0), 0, 0, 0, 0, L"Redbull", 0 };
    RegisterClassEx(&wc);
    Window = CreateWindow(wc.lpszClassName, L"Redbull", WS_POPUP, 0, 0, 5, 5, 0, 0, wc.hInstance, 0);
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        Menu::Theme();
        {
            ImGui::Begin("Redbull Menu", 0, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoResize);
            {
                Menu::Render();
            }
            ImGui::End();
